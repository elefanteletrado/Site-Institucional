 ...
 * Time: 9:12 AM
 */
if ( ! function_exists( 'cupid_get_breadcrumb' ) ) {
    function cupid_get_breadcrumb() {

        $items = cupid_get_breadcrumb_items();
...
if ( ! function_exists( 'cupid_get_breadcrumb_items' ) ) {
    function cupid_get_breadcrumb_items() {
        global $wp_query;

        $item = array();
        /* Front page. */
        if ( is_front_page() ) {
            $item['last'] = __( 'Home', 'cupid' );
        }


        /* Link to front page. */
        if ( !is_front_page() ) {
            $item[] = '<li typeof="v:Breadcrumb"><a rel="v:url" property="v:title" href="'. home_url( '/' ) .'" class="home">' . __( 'Home', 'cupid' ) . '</a></li>';
        }


        /* If bbPress is installed and we're on a bbPress page. */
        if ( function_exists( 'is_bbpress' ) && is_bbpress() ) {
            $item = array_merge( $item, cupid_breadcrumb_get_bbpress_items() );
        }

        elseif ( function_exists( 'is_woocommerce' ) && is_woocommerce() ) {
            $item = array_merge( $item, cupid_filter_breadcrumb_items() );
        }

        /* If viewing a home/post page. */
        elseif ( is_home() ) {
            $home_page = get_post( $wp_query->get_queried_object_id() );
            $item = array_merge( $item, cupid_breadcrumb_get_parents( $home_page->post_parent ) );
        ...
        elseif ( is_singular() ) {
            ...
            if ( 'post' === $wp_query->post->post_type ) {
                ...
                $item = array_merge( $item, cupid_breadcrumb_get_term_parents( $categories[0]->term_id, $categories[0]->taxonomy ) );
            }
            ...
            if ( 'page' !== $wp_query->post->post_type ) {
                ...
                if ( isset( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) && is_taxonomy_hierarchical( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) ) {
                    ...
                    $item = array_merge( $item, cupid_breadcrumb_get_term_parents( $terms[0], $args["singular_{$wp_query->post->post_type}_taxonomy"] ) );
                }
            ...
            if ( ( is_post_type_hierarchical( $wp_query->post->post_type ) || 'attachment' === $wp_query->post->post_type ) && $parents = cupid_breadcrumb_get_parents( $wp_query->post->post_parent ) ) {
        ...
        /* If viewing any type of archive. */
            ...
            if ( is_category() || is_tag() || is_tax() ) {
                ...
                if ( ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent ) && $parents = cupid_breadcrumb_get_term_parents( $term->parent, $term->taxonomy ) )
                    $item = array_merge( $item, $parents );
                ...
                if ( is_day() )
                    $item['last'] = __( 'Archives for ', 'cupid' ) . get_the_time( 'F j, Y' );

                elseif ( is_month() )
                    $item['last'] = __( 'Archives for ', 'cupid' ) . single_month_title( ' ', false );

                elseif ( is_year() )
                    $item['last'] = __( 'Archives for ', 'cupid' ) . get_the_time( 'Y' );
            }

            else if ( is_author() )
                $item['last'] = __( 'Archives by: ', 'cupid' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
        }

        /* If viewing search results. */
        else if ( is_search() )
            $item['last'] = __( 'Search results for "', 'cupid' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

        /* If viewing a 404 error page. */
        else if ( is_404() )
            $item['last'] = __( 'Page Not Found', 'cupid' );
...
if ( ! function_exists( 'cupid_filter_breadcrumb_items' ) ) {
...
if ( ! function_exists( 'cupid_breadcrumb_get_bbpress_items' ) ) {
...
if ( ! function_exists( 'cupid_breadcrumb_get_parents' ) ) {
    function cupid_breadcrumb_get_parents( $post_id = '', $separator = '/' ) {
        $parents = array();
...
if ( ! function_exists( 'cupid_breadcrumb_get_term_parents' ) ) {
    function cupid_breadcrumb_get_term_parents( $parent_id = '', $taxonomy = '', $separator = '/' ) {
        $parents = array();
