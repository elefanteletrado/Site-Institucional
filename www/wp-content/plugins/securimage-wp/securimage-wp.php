    ...
    if (!$forceDisplay && is_user_logged_in() && current_user_can('administrator')) {
        ...
        if ($count < 1) {
            ...
            function siwp_refresh(id, audioObj) {
                ...
                document.getElementById('siwp_captcha_image_' + id).src = '{$plugin_url}lib/siwp_captcha.php?id=' + cid;
                if (null !== audioObj) {
        ...
        $captcha_html .= "<script type=\"text/javascript\">var siwp_interval = setInterval(function() { siwp_refresh('{$count}', $audio_obj); }, $timeout);</script>";
        ...
        foreach($sequence as $part) {
            switch($part) {
                ...
                case 'captcha':
                {
                    ...
                    if (!$disable_audio) {
                        // html5 audio
                        $play_path = siwp_get_plugin_url() . 'lib/siwp_play.php?id=' . $captchaId;

                        $captcha_html .=
                            sprintf('<div id="%s_audio_div">', $imgTagId) . "\n" .
                            sprintf('<audio id="%s_audio" preload="none" style="display: none">', $imgTagId) . "\n";

                        // check for existence and executability of LAME binary
                        // prefer mp3 over wav by sourcing it first, if available
                        if (!empty($lame_binary_path) && is_executable($lame_binary_path)) {
                            $captcha_html .= sprintf('<source id="%s_source_mp3" src="%s&amp;format=mp3" type="audio/mpeg"></source>', $imgTagId, $play_path) . "\n";
                        }

                        // output wav source
                        $captcha_html .= sprintf('<source id="%s_source_wav" src="%s" type="audio/wav"></source>', $imgTagId, $play_path) . "\n";
                        ...
                        $captcha_html .=
                            sprintf('<div id="%s_audio_controls">', $imgTagId) . "\n" .
                            sprintf('<a class="captcha_play_button" href="%s" tabindex="-1" onclick="return false">', $play_path) . "\n" .
...
function siwp_get_code_from_database($captchaId)
{
    ...
    $result = $wpdb->get_row(
        $wpdb->prepare("SELECT id, code, code_display, created FROM $table_name WHERE id = %s", $captchaId)
    );
...
function siwp_get_audio_from_database($captchaId)
{
    global $wpdb;
    $table_name = siwp_get_table_name();

    $result = $wpdb->get_row(
        $wpdb->prepare("SELECT audio_data FROM $table_name WHERE id = %s", $captchaId)
    );
...
function siwp_save_audio_to_database($captchaId, $audio)
{
    global $wpdb;
    $table_name = siwp_get_table_name();

    $wpdb->query(
        $wpdb->prepare("UPDATE $table_name SET audio_data = %s WHERE id = %s", $audio, $captchaId)
    );
}
...
// register securimage audio js
if (false == get_option('siwp_disable_audio')) {
    add_action('wp_enqueue_scripts', 'siwp_register_scripts');

    if (true == get_option('siwp_enabled_loginform', 0)) {
        add_action('login_enqueue_scripts', 'siwp_register_scripts');
    }
}
...
?>
    ...
    <table class="form-table">
        ...
        <tr valign="top">
            <th scope="row"><label for="siwp_disable_flashfallback"><?php _e('Disable Flash Audio Fallback') ?></label><span style="font-size: 0.8em"><br /><?php _e('HTML5 audio is used by default but optional Flash fallback can be used by browsers that do not support HTML5 audio (See FAQ for more info).') ?></span></th>
        ...
        <tr valign="top">
            <th scope="row"><label for="siwp_lame_binary_path"><?php _e('LAME Encoder Path') ?></label><span style="font-size: 0.8em"><br /><?php _e('Audio is output in WAV format unless the LAME encoder is installed.  Internet Explorer 9+ support HTML5 audio but do not support WAV format.  If LAME is not installed Flash will be used unless Flash fallback is disabled, in which case the audio icon will be hidden.') ?></span></th>
            <td>
               ...
               <?php _e('The LAME encoder binary was not found on your system.  In order to support MP3 audio you must install LAME and configure the path here.  See FAQ for details.') ?>
               <?php else: ?>
               <?php _e('The path to LAME is correctly set.') ?>
               <?php endif; ?>
            </td>
        </tr>
        ...
        <tr valign="top">
            <th scope="row"><?php _e('Audio Button Icon URL') ?><br /><span style="font-size: 0.8em"><?php _e('For best results, this should be hosted on the same domain as WordPress') ?></span></th>
            <td>
        ...
        <tr valign="top">
            <th scope="row"><?php _e('Audio Loading Icon URL') ?><br /><span style="font-size: 0.8em"><?php _e('For best results, this should be a spinner icon (PNG format) that will be rotated using CSS3 transitions') ?></span></th>
            <td>
        ...
        <tr valign="top">
            <th scope="row"><?php _e('Flash Button Background Color') ?><br /><span style="font-size: 0.8em"><?php _e('Background color to use for Flash button') ?></span></th>
