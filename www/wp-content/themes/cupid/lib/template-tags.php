...
if (!function_exists('cupid_comment_form')) :
    function cupid_comment_form() {
        ...
        $fields   =  array(
            ...
            'author' => '<div>'.
                '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '"  placeholder="'.__('Nome','cupid').'" '. $aria_req.'>' .
                '</div>',
            ...
            'email' => '<div>'.
                ...
                '</div>',
            'url' => '<div>'
                // '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '"  placeholder="'.__('Site','cupid') .'" '. $aria_req.'>' .
                // '</div>'
        );
        ...
        $comment_form_args = array(
            'fields'               => $fields,
            'comment_field' => '<div>'.
                '<textarea rows="6" id="comment" name="comment"  value="' . esc_attr( $commenter['comment_author_url'] ) . '"  placeholder="'.__('Mensagem','cupid') .'"  aria-required="true"></textarea>' .
                '</div>',


            'comment_notes_before' => '<p class="comment-notes">' .
                __('O seu e-mail não vai ser publicado!', 'cupid') /* . ( $req ? $required_text : '' ) */ .
                '</p>',
            'comment_notes_after'  => '',
            'id_submit'            => 'btnComment',
            'class_submit'          => 'cupid-button button-sm',
            'title_reply'          => __( 'Deixe um Comentário!','cupid' ),
            'title_reply_to'       => __( 'Deixe um Comentário!','cupid' ),
            'cancel_reply_link'    => __('Cancelar','cupid'),
            'label_submit'         => __('Enviar','cupid')
        );
...
if (!function_exists('cupid_post_meta')) :
    function cupid_post_meta() {
        ...
        <span class="entry-meta-author">
                <?php _e("Postado por",'cupid'); ?> <?php printf('<a href="%1$s">%2$s</a>',esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),esc_html( get_the_author() )); ?>
        </span>
        <span class="entry-meta-date">
            <?php _e("",'cupid'); ?> <a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"> <?php echo  get_the_date('d/m/Y');?> </a>
        </span>
...
if (!function_exists('cupid_post_nav')) {
    function cupid_post_nav() {
        ...
        <nav class="post-navigation" role="navigation">
            <div class="nav-links">
                <?php
                previous_post_link( '<div class="nav-previous">%link</div>', _x( '<i class="post-navigation-icon fa fa-angle-left"></i> <span class="post-navigation-content"><span class="post-navigation-label">Post Anterior</span> <h4 class="post-navigation-title">%title </h4> </span> ', 'Post Anterior', 'cupid' ) );
                next_post_link( '<div class="nav-next">%link</div>', _x( '<i class="post-navigation-icon fa fa-angle-right"></i> <span class="post-navigation-content"><span class="post-navigation-label">Próximo Post</span> <h4 class="post-navigation-title">%title</h4></span> ', 'Próximo post', 'cupid' ) );
                ?>
            </div>
            <!-- .nav-links -->
        </nav><!-- .navigation -->
