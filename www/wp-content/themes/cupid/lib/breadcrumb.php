    ...
    function cupid_get_breadcrumb_items() {
        ...
        /* If viewing any type of archive. */
            ...
            if ( is_category() || is_tag() || is_tax() ) {
                ...
                if ( is_day() )
                    $item['last'] = __( 'Arquivo ', 'cupid' ) . get_the_time( 'F j, Y' );

                elseif ( is_month() )
                    $item['last'] = __( 'Arquivo ', 'cupid' ) . single_month_title( ' ', false );

                elseif ( is_year() )
                    $item['last'] = __( 'Arquivo ', 'cupid' ) . get_the_time( 'Y' );
            }

            else if ( is_author() )
                $item['last'] = __( 'Arquivo por: ', 'cupid' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
        }

        /* If viewing search results. */
        else if ( is_search() )
            $item['last'] = __( 'Resultados para "', 'cupid' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

        /* If viewing a 404 error page. */
        else if ( is_404() )
            $item['last'] = __( 'Página não encontrada', 'cupid' );
