...
function cupid_result_search_callback() {
	...
	if ( $keyword ) {
		$search_query = array(
			...
			'orderby'   	=> 'date',
			'post_status'	=> 'publish',
			'post_type' 	=> array('post','cupid_classes'),
			'nopaging' => true,
		);
		...
		if ($search && count($search->post) > 0) {
			...
			$newdata[] = array(
				'id'        => -1,
				'title'     => __('Sorry, but nothing matched your search terms. Please try again with different keywords.','cupid'),
				'guid'      => '',
				'date'      => null,
				'shortdesc' => ''
			);
...
add_action( 'wp_ajax_nopriv_result_search', 'cupid_result_search_callback' );
add_action( 'wp_ajax_result_search', 'cupid_result_search_callback' );