Popup_icon = {
	...
	close : function() {
		//tb_remove();
        jQuery('#cupid-popup-icon-wrapper').remove();

	},
	setPreview : function() {
		jQuery('.list-icon ul li a',Popup_icon.htmlTag.wrapper).removeClass('active');
		if (Popup_icon.vars.current_icon != '') {
			jQuery(Popup_icon.htmlTag.iconPreview + ' i',Popup_icon.htmlTag.wrapper).attr('class','fa ' + Popup_icon.vars.current_icon);
...
jQuery(document).ready(function(){
	setTimeout(function() {
		...
		jQuery(document).on('change','.input-icon',function(){
			...
			if (icon != '') {
				jQuery('.icon-preview > i',jQuery(this).parent()).attr('class','fa ' + icon);
